...
delete_files(__DIR__.'/product/');
mkdir(__DIR__.'/product/', 0777, true);

// save data to mxl file
function saveXmlSitemap($xmlRows, $j)
{
	$_fnc = ($j > 1) ? '-'.$j : '';
	$file_name = 'product/products'.$_fnc.'.xml';
...
$sql = "SELECT id, post_name, post_modified, post_parent FROM {$wpdb->prefix}posts WHERE post_type='product' AND post_status='publish' ORDER BY id";
